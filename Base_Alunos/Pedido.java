...
public class Pedido implements Comparable<Pedido> {
	...
	public ItemDePedido existeNoPedido(Produto produto) {
		...
		return itensDePedido.buscarPor((item1, item2) -> {
			if (item1.equals(item2))
				return 0;
			return 1;
		}, itemDePedidoProcurado);
	}

	/**
	 * Inclui produtos no pedido. Se necessário, aumenta a quantidade de itens
	 * armazenados no pedido até o momento.
	...
	/**
	 * Calcula e retorna o valor final do pedido (soma do valor de venda de todos os
	 * itens do pedido).
	 * Caso a forma de pagamento do pedido seja à vista, aplica o desconto
	 * correspondente.
	 */
	public double valorFinal() {
		...
		if (!itensDePedido.vazia()) {
			valorPedido = itensDePedido.somarMultiplicacoes(item -> (double) item.getQuantidade(), item -> 1);
			for (ItemDePedido item : itensDePedido) {
				valorPedido += item.getQuantidade() * item.getPrecoVenda();
			}
		}
